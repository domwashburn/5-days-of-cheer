...
gulp.task('browser-sync', function() {
    browserSync.init(['./assets/css/*.css', './assets/js/*.js', './*.html'], {        // files to inject
        server: {
            host: "local.dev",
            baseDir: "./"
        }
    })
    .pipe(notify({ message: 'Your browser has been synchronized!'}));     // notify when done
});
